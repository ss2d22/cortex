...
class CactusService extends ChangeNotifier {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      debugPrint('CactusService initialized');
    } catch (e) {
    ...
    try {
      await tempStt.downloadModel(
        ...
        downloadProcessCallback: (progress, status, isError) {
          if (!isError) onProgress?.call('STT', progress ?? 0, status);
        },
      );
      onProgress?.call('STT', 1.0, 'Voice model ready');
    } catch (e) {
      debugPrint('STT pre-download error: $e');
  ...
  Future<void> _predownloadVision() async {
    ...
    try {
      await tempVision.downloadModel(
        ...
        downloadProcessCallback: (progress, status, isError) {
          if (!isError) onProgress?.call('Vision', progress ?? 0, status);
        },
      );
      onProgress?.call('Vision', 1.0, 'Vision model ready');
    } catch (e) {
      debugPrint('Vision pre-download error: $e');
  ...
  Future<CactusCompletionResult> generateCompletion({
    ...
    return await _primaryLM!.generateCompletion(messages: messages, params: params);
  }
  ...
  Future<CactusStreamedCompletionResult> generateCompletionStream({
    ...
    return await _primaryLM!.generateCompletionStream(messages: messages, params: params);
  }

  Future<CactusLM> getVisionLM() async {
    if (_visionInitialized && _visionLM != null) return _visionLM!;
    ...
    await _visionLM!.downloadModel(
      model: AppConstants.visionModel,
      downloadProcessCallback: (progress, status, isError) {
        if (!isError) onProgress?.call(AppConstants.visionModel, progress ?? 0, status);
      },
    );
  ...
  Future<CactusSTT> getSTT() async {
    if (_sttInitialized && _stt != null) return _stt!;
    ...
    await _stt!.downloadModel(
      model: AppConstants.sttModel,
      downloadProcessCallback: (progress, status, isError) {
        if (!isError) onProgress?.call(AppConstants.sttModel, progress ?? 0, status);
      },
    );
  ...
  @override
  void dispose() {
    try { _primaryLM?.unload(); } catch (_) {}
    try { _embeddingLM?.unload(); } catch (_) {}
    try { _visionLM?.unload(); } catch (_) {}
    try { _stt?.unload(); } catch (_) {}
    _rag?.close();
    super.dispose();
  }
}
